...
class NonLocalBlockBase(nn.Module):
    def __init__(self, in_channels, inter_channels=None, dimension=3, sub_sample=True, bn_layer=True):
        ...
        # dimension == 3, T*H*W, not D*H*W, T stands for temporal dependency
        if dimension == 3:
            conv_nd = nn.Conv3d
...
class MultiViewNonLocalBlockBase(nn.Module):
    def __init__(self, n_views, in_channels, inter_channels = None, dimension = 3, sub_sample = True, bn_layer = True):
        ...
        self.blocks = [ NonLocalBlockBase(in_channels[i], inter_channels[i] if inter_channels is not None else None, dimension, sub_sample, bn_layer) for i in range(self.n_views) ]
